...
class Pos extends MY_Controller
{
    ...
    public function add_payment($id = null)
    {
        ...
        if (true == $this->form_validation->run()) {
            ...
            if ('deposit' == $this->input->post('paid_by')) {
            ...
            $payment = [
                ...
                'cc_no'        => 'gift_card' == $this->input->post('paid_by') ? $this->input->post('gift_card_no') : $this->input->post('pcc_no'),
        ...
        if (true == $this->form_validation->run() && $msg = $this->pos_model->addPayment($payment, $customer_id)) {
            if ($msg) {
                if (0 == $msg['status']) {
                    unset($msg['status']);
                    $error = '';
    ...
    public function add_printer()
    {
        ...
        if ('network' == $this->input->post('type')) {
        ...
        if (true == $this->form_validation->run()) {
            $data = ['title'    => $this->input->post('title'),
                ...
                'port'          => ('network' == $this->input->post('type')) ? $this->input->post('port') : null,
            ];
        }

        if (true == $this->form_validation->run() && $cid = $this->pos_model->addPrinter($data)) {
    ...
    public function mpesapay()
    {
        ...
        $id = $post->TransID;
        $this->db->update('mpesatrans', ['status' => 'paid'], ['TransID' => $id]);
        $this->sma->send_json(['status' => 'ok']);
    }
    public function mpesa()
    {
        ...
        if ($q->num_rows() > 0) {
                ...
                if (isset($data))
                {
                $this->sma->send_json(['trans' => 'yes','data' => $data]);
                }
                else {
    ...
    public function edit_printer($id = null)
    {
        ...
        if (true == $this->form_validation->run() && $this->pos_model->updatePrinter($id, $data)) {
    ...
    public function getProductDataByCode($code = null, $warehouse_id = null)
    {
        ...
        if ($row) {
            ...
            if ('standard' == $row->type && (!$this->Settings->overselling && $row->quantity < 1)) {
                echo null;
                die();
            }
            if ($options) {
    ...
    public function index($sid = null)
    {
        ...
        if (true == $this->form_validation->run()) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_option = isset($_POST['product_option'][$r]) && 'false' != $_POST['product_option'][$r] ? $_POST['product_option'][$r] : null;
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    $product_details = 'manual' != $item_type ? $this->pos_model->getProductByCode($item_code) : null;
                    // $unit_price = $real_unit_price;
                    if ('digital' == $item_type) {
                        $digital = true;
                    }
                    ...
                    if (isset($item_tax_rate) && 0 != $item_tax_rate) {
                        $tax_details = $this->site->getTaxRateByID($item_tax_rate);
                        $ctax = $this->site->calculateTax($product_details, $tax_details, $unit_price);
                        $item_tax = $this->sma->formatDecimal($ctax['amount']);
                        $tax = $ctax['tax'];
                        if (!$product_details || (!empty($product_details) && 1 != $product_details->tax_method)) {
            ...
            } elseif (0 == $this->pos_settings->item_order) {
                krsort($products);
            }
            ...
            if (!$suspend) {
                ...
                for ($r = 0; $r < $p; $r++) {
                    if (isset($_POST['amount'][$r]) && !empty($_POST['amount'][$r]) && isset($_POST['paid_by'][$r]) && !empty($_POST['paid_by'][$r])) {
                        ...
                        if ('deposit' == $_POST['paid_by'][$r]) {
                            if (!$this->site->check_customer_deposit($customer_id, $amount)) {
                                $this->session->set_flashdata('error', lang('amount_greater_than_deposit'));
                                redirect($_SERVER['HTTP_REFERER']);
                            }
                        }
                        if ('gift_card' == $_POST['paid_by'][$r]) {
        ...
        if (true == $this->form_validation->run() && !empty($products) && !empty($data)) {
            if ($suspend) {
                ...
                if ($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
                    ...
                    if ($this->pos_settings->auto_print) {
                        if (1 != $this->Settings->remote_printing) {
    ...
    public function settings()
    {
        ...
        if (true == $this->form_validation->run() && $this->pos_model->updateSetting($data)) {
            if (DEMO) {
