...
<body>
                ...
                <div class="pos">
                    ...
                    <div id="pos">
                        ...
                        <div id="leftdiv">
                            <div id="leftdiv_">
                                <div id="printhead">
                                    <h4 style="text-transform:uppercase;">CLONE TECH</h4>
                                    <?php
                            ...
                            <div id="button_">
                                <div id="botbuttons" class="right-button text-center">
                                                                                ...
                                                                                <div class="col-sm-11">
                                                                                    ...
                                                                                    <div class="pcheque_1" style="display:none;">
                                                                                        <div class="form-group"><?= lang('cheque_no', 'cheque_no_1'); ?>
                                                                                            <input name="mpesa_no[]" type="text" id="mpesa_no_1" class="form-control cheque_no" />
                                                                                        </div>
                                                                                    </div>
                                                                                    <div class="pmpesa_1 pmpesa" style="display:none;">
                                                                                        <div class="form-group"><label for="mpesa_no_1">Customer No</label>
                                                                                            <input name="mpesa_no[]" type="number" id="mpesano_1" class="form-control" />
                                                                                        </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.pmpesa .mpesabtn', function(event) {
                ...
                $mpesaBtn.text('Reload');
                $id = $mpesaBtn.attr('id');
                $mNumber = $mpesaBtn.parent();
                $('#submit-sale').text('<?= lang('confirm_mpesa'); ?>').attr('disabled', true);

                let $amountid = "#amount_" + $id.split('_')[1];
                let $amount = $("#amount_" + $id.split('_')[1]).val();
                let cusno = $("#mpesano_" + $id.split('_')[1]).val();
                let idNum = $id.split('_')[1];
                if ($('#mpesadisp_' + idNum).length) {
                    $('#mpesadisp_' + idNum).remove();
                }
                    ...
                    success: function(data) {
                        if (data['trans'] == 'yes') {
                            ...
                            for (let i = 0; i < data1.length; i++) {
                                ...
                                let blem = jQuery('<button>', {
                                    id: data1[i]['TransID'],
                                    class: 'btn btn-primary col-md-2 mpesapay'
                                });
                                blem.text('pay');
            ...
            $(document).on('click', '.mpesapay', function(event) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data['status'] == 'ok') {
                            $('#submit-sale').text('<?= lang('submit'); ?>').attr('disabled', false);
                            $parent = $mpesaBtn.parent();
                            // $parent.siblings().each( function () {
                            // var tx=$(this).css();
                            //     alert(tx);
                            //     });
                            $parent.siblings().css("display", "none");
                            $mpesaBtn.text('<?= lang('done'); ?>').attr('disabled', true);
                            $mpesaBtn.removeClass("btn-primary");
                            $mpesaBtn.addClass("btn-success");
                            let parid = $parent.parent().attr('id');
                            // console.log(parid);
                            let $btnid = "#mbtn_" + parid.split('_')[1];
                            $($btnid).hide();
                        }
            ...
            $(document).on('click', '.addButton', function() {
                if (pa <= 5) {
                    ...
                    if (bal > 0) {
                        ...
                        if ($('#mpesadisp_' + pa).length) {
                            $('#mpesadisp_' + pa).remove();
                        }
                        let $btnid = "#mbtn_" + pa;
                        $($btnid).text('<?= lang('mpesa'); ?>').show();
    ...
    ?>
        ...
        <script type="text/javascript">
            socket = io.connect('http://localhost:6440', {
                'reconnection': false
            });
